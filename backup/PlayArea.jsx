...
function PlayArea({setEffect, bloom}) {
  ...
  const { startOrStop } = useControls({" ": buttonGroup({
    "start": () => {
      if (!gameRef.current) {
  ...
  const { clear } = useControls({ clear : button(() => {
    const emptyBoard = [];
    for (let i = 0; i < configuration.length; i++) {
  ...
  //const { size } = useControls({ size: { value: 16, min: 5, max: 40, step: 1 } });

  const { size } = useControls({"  ": buttonGroup({
    "size +": () => {
      setGameParameters(prev => {
        console.log(prev.configuration);
        const newConfig = [...prev.configuration].map(m => m);
        const expanded = newConfig.map(arr => [0, ...arr, 0]);
        ...
        return {...prev, isRunning: false, configuration: expanded, sizeModifier: prev.sizeModifier - 1};
      })
    ...
    "size -": () => {
     ...
     // setGameParameters(prev => {
       // return {...prev, isRunning: false, configuration: newConfig, sizeModifier: prev.sizeModifier - 1}
      //});
    }
  })});
